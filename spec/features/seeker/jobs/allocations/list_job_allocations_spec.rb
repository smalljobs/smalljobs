...
feature 'List the job allocations' do
  ...
  scenario 'displays the job allocations for the seeker' do
  ...
  scenario 'does not display the job allocations for the other seeker' do
    visit_on seeker_A, '/seeker/dashboard'
    ...
    expect(page).to_not have_content 'Achtung, nur bei schönem Wetter'
  end
