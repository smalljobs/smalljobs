...
describe 'broker/jobs/edit.html.haml' do
  ...
  context 'form elements' do
    it 'renders the assignment input' do
      expect(rendered).to have_field('Anbieter', with: provider.id)
  ...
  context 'global actions' do
    it 'contains the link to show the job' do
      expect(rendered).to have_link('Anzeigen', broker_job_path(job))
    end
