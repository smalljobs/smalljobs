...
describe 'seeker/jobs/index.html.haml' do
    ...
    context 'list items' do
      ...
      context 'list item data' do
        it 'contains the link to show the jobs details' do
          expect(rendered).to have_link('Job A anzeigen', href: seeker_job_path(job_A))
          expect(rendered).to have_link('Job B anzeigen', href: seeker_job_path(job_B))
        end
      end
    end
  end
