...
describe JobProvider do
  ...
  describe '#email_required?' do
    it 'does not require an email' do
      expect(Fabricate(:job_provider).email_required?).to be_false
    end
  end
  ...
  describe '#active_for_authentication?' do
    ...
    it 'is not active when not activated' do
      expect(Fabricate(:job_provider, confirmed: true, active: false).active_for_authentication?).to be_false
    end

    it 'is active when activated and confirmed' do
      expect(Fabricate(:job_provider, confirmed: true, active: true).active_for_authentication?).to be_true
    end
  end

  describe '#contact_preference_enum' do
    it 'returns the list of contact strings' do
      expect(Fabricate(:job_provider).contact_preference_enum).to eq(%w(email phone mobile postal))
    end
  end
