class Seeker < ActiveRecord::Base
  ...
  has_many :broadcast_message_seekers, dependent: :destroy
  has_many :broadcast_messages, through: :broadcast_message_seekers
  ...
  def send_welcome_message
    ...
    message = Mustache.render(
      ...
      broker_first_name: self.organization.broker.present? ? self.organization.broker.try(:firstname).to_s : self.organization.brokers.first.try(:firstname).to_s,
      broker_last_name: self.organization.broker.present? ? self.organization.broker.try(:lastname).to_s : self.organization.brokers.first.try(:lastname).to_s,
    ...
    begin
      MessagingHelper::send_message(self.organization.broker.present? ? self.organization.broker.try(:rc_id) : self.organization.brokers.first.try(:rc_id), self.rc_username, "#{title}. #{message}")
