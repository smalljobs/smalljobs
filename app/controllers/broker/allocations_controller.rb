class Broker::AllocationsController < InheritedResources::Base
  ...
  def show
    ...
    @get_job_msg = Mustache.render(current_region.organizations.first.get_job_msg  || '', seeker_first_name: @allocation.seeker.firstname || '', seeker_last_name: @allocation.seeker.lastname || '', seeker_link_to_agreement: url_for(agreement_broker_seeker_url(@allocation.seeker.agreement_id)) || '', provider_first_name: @job.provider.firstname || '', provider_last_name: @job.provider.lastname || '', provider_phone: job_provider_phone || '', broker_first_name: current_broker.firstname || '', broker_last_name: current_broker.lastname || '', organization_name: @org.name || '', organization_zip: @org.place.zip || '', organization_street: @org.street || '', organization_place: @org.place.name || '', organization_phone: @org.phone || '', organization_email: @org.email || '', link_to_jobboard_list: url_for(root_url()) || '', job_title: @job.title)
    @get_job_msg.gsub! "\n", "<br>"
